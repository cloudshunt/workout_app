  ...
  async copyFromSetupToTrack(routineName, dayNum, routineId, sessionName) {
    ...
    session_exercises AS (
      ...
      SELECT 
        uuid_generate_v4(), 
        sessions.id, 
        setup_session_exercises.exercise_id, 
        custom_exercise_id, 
        setup_session_exercises.exercise_order, 
        setup_session_exercises.exercise_comment, 
        setup_session_exercises.id
      FROM setup_session_exercises
    ...
    INSERT INTO track_exercise_details 
      (id, track_session_exercise_id, weight, cur_set, reps_goal, myo_order, setup_id)
    SELECT 
      uuid_generate_v4(), 
      session_exercises.id, 
      setup_exercise_details.weight, 
      setup_exercise_details.cur_set, 
      setup_exercise_details.reps_goal, 
      setup_exercise_details.myo_order,
       setup_exercise_details.id
    FROM setup_exercise_details
    JOIN session_exercises 
      ON setup_exercise_details.setup_session_exercise_id = session_exercises.setup_id;
      `;
    

    await dbQuery(COPY_QUERY, this.userId, routineName, dayNum, routineId, sessionName);


    // // Remove setup fields from all track tables that are done copying over from setup
    // // This allows user to make edit/delete routines that are in setup tables
    const REMOVE_SETUP_FIELDS = `
      WITH routine_id AS (
          UPDATE track_routines
          SET setup_id = NULL
          WHERE setup_id IS NOT NULL AND user_id = $1
          RETURNING id
      ),
      day_id AS (
          UPDATE track_days
          SET setup_id = NULL
          WHERE track_routine_id IN (SELECT id FROM routine_id)
          RETURNING id
      ),
      session_id AS (
      ...
      session_exercise_ids AS (
          UPDATE track_session_exercises
          SET setup_id = NULL
          WHERE track_session_id IN (SELECT id FROM session_id)
          RETURNING id
      )
      UPDATE track_exercise_details
      SET setup_id = NULL
      WHERE track_session_exercise_id IN (SELECT id FROM session_exercise_ids)
      RETURNING id
    `

    await dbQuery(REMOVE_SETUP_FIELDS, this.userId);
  }
