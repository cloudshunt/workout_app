...
async countDays(routineId) {
  ...
  async deleteDayAndSessionShiftDays(routineId, dayNumber) {
    // Step 1: Delete sessions associated with the specified day in `setup_sessions`
    const DELETE_SESSIONS_QUERY = `
      DELETE FROM setup_sessions
      WHERE id IN (
        SELECT session_id FROM setup_days_sessions
        WHERE day_id = (
          SELECT id FROM setup_days 
          WHERE setup_routine_id = $1 AND day_number = $2
        )
      );
    `;
    await dbQuery(DELETE_SESSIONS_QUERY, routineId, dayNumber);

    // Step 2: Delete the specified day from `setup_days`
    ...
    // Step 3: Shift subsequent days by decrementing their day_number by 1
